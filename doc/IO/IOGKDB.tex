...
\end{eqnarray}
where $f_s^{gc}$ is the guiding center perturbed distribution function of species $s$ (see Sec.~\ref{sec:gctogy} for the relationship with the gyro-center distribution function $f_s$). The three terms in the square brackets are the electrostatic, magnetic flutter and magnetic compression drifts, respectively:
\begin{equation}
...
$$\chi= \left<\phi\right>_\textrm{gy} - v_\parallel \left<A_{\parallel}\right>_\textrm{gy} + \frac{\mu}{Z_s e}\left<B_{\parallel}\right>_\textrm{gy}$$ 
and noting that 
\begin{equation*}
\left< \textrm{Re}\left[f_s\right] \textrm{Re}\left[\frac{\partial \chi}{\partial y}\right]\right>  = \sum_{k_x,k_y>0}\left< 2  k_y \textrm{Im}\left[\hat{f_s}(k_x,k_y,\theta)\hat{\chi}^\dagger(k_x,k_y,\theta)\right] \right>  
\end{equation*}
the fluxes can be expressed as
\begin{align*}
\Gamma_s & = \sum_{k_x,ky>0} \Gamma_s(k_x,k_y) = \sum_{k_x,ky>0}  \left<2k_y\frac{\mathbf{b}\times \nabla y}{B}\cdot \nabla r \int \textrm{Im}\left[\hat{f}_s\hat{\chi}^\dagger \right] \,\textrm{d}\mathbf{v}\right>\\
\end{align*}

\subsection{Normalisation}
The fluxes given for each Fourier mode are normalised to the corresponding mode amplitude (taken to be 1 for non-linear runs) and made dimensionless as follows:
\begin{eqnarray}
...
\end{eqnarray}
In addition, the electrostatic, magnetic flutter and magnetic compression contributions are stored separately (e.g. $\Gamma_{sN}=\Gamma_{sN,\phi}+\Gamma_{sN,A_\parallel}+\Gamma_{sN,B_\parallel}$) for each flux. The fluxes are normalised with respect to $n_s(\theta=0)$ rather than $n_\tref$ to allow for trace species with zero density.
\begin{align*}
...
For non-linear runs, the total fluxes are defined as the sum of the fluxes over all wavevectors included in the simulation:
\begin{eqnarray}
\Gamma_{sN}^\textrm{tot} &=& \sum_{k_x, k_y} \Gamma_{sN}(k_x,k_y)\\
\Pi_{sN}^\textrm{tot} &=&  \sum_{k_x, k_y}\Pi_{sN}(k_x,k_y)\\
Q_{sN}^\textrm{tot} &=&  \sum_{k_x, k_y} Q_{sN}(k_x,k_y) 
\end{eqnarray}
...
\section{Simplified shape description} \label{sec:shape}
...
\begin{align*}
\Gamma_s^\textrm{pol} & = - \left< \frac{Z_s e}{T_s} \int F_{Ms} [ \phi - \left< \phi \right>_\textrm{gy} ]\mathbf{v}_\chi\cdot \nabla r  \,\textrm{d}\mathbf{v}\right>\\
\Gamma_s^\textrm{mag} & = - \left< \frac{1}{T_s} \int \mu F_{Ms} \left< B_\parallel \right>_\textrm{gy} \mathbf{v}_\chi\cdot \nabla r  \,\textrm{d}\mathbf{v}\right> \\
\Pi_s^\textrm{pol} & = - \left< s_bm_sR\frac{|B_t|}{B}\frac{Z_s e}{T_s} \int v_\parallel F_{Ms} [ \phi - \left< \phi \right>_\textrm{gy} ]\mathbf{v}_\chi\cdot \nabla r  \,\textrm{d}\mathbf{v}\right>\\
\Pi_s^\textrm{mag} & = - \left< s_bm_sR\frac{|B_t|}{B}\frac{1}{T_s} \int \mu v_\parallel F_{Ms} \left< B_\parallel \right>_\textrm{gy} \mathbf{v}_\chi\cdot \nabla r  \,\textrm{d}\mathbf{v}\right>\\
Q_s^\textrm{pol} & = - \left< \frac{1}{2}m_s\frac{Z_s e}{T_s} \int \mu v^2 F_{Ms} [ \phi - \left< \phi \right>_\textrm{gy} ]\mathbf{v}_\chi\cdot \nabla r  \,\textrm{d}\mathbf{v}\right>\\
Q_s^\textrm{mag} & = - \left< \frac{1}{2}m_s\frac{1}{T_s} \int \mu v^2 F_{Ms} \left< B_\parallel \right>_\textrm{gy} \mathbf{v}_\chi\cdot \nabla r  \,\textrm{d}\mathbf{v}\right>
\end{align*}
Then use the following relationship and the spectral representation to simplify the expressions above:
To be completed...
 
\subsection{Fluxes and gyro-averaged moments}
The next step is to rewrite the expressions of the gyro-center fluxes as a function of the gyro-averaged moments. 
